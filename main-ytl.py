...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda:0', help='cuda 0, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--profile', action='store_true', help='profile model speed')
    parser.add_argument('--line-profile', action='store_true', help='profile model speed layer by layer')
    parser.add_argument('--test', action='store_true', help='test all yolo*.yaml')
    parser.add_argument('--data', type=str, default='mydata-flir-hotcold.yaml', help='dataset.yaml path')
    ...
    for epoch in range(opt.epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            pso.optimize(func)
            swarm_parameters,results = pso.run()
            plt.plot(results)
            plt.savefig('res{}'.format(i))
            plt.show()
