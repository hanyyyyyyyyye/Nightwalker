...
class ParticleToPatch():
    ...
    def __call__(self, x, targets, imgs):#调用这个类实际上是调用call函数 x是对抗扰动得参数
        bs, c, h, w = imgs.size()

        patch_size_h = 90
        patch_size_w = 90
        patch = torch.ones((3, patch_size_w, patch_size_h), dtype=torch.float32).cuda() * 0.3#patch初始是一个(3,90,90)的矩阵元素都是0.3
        patch_mask = torch.ones_like(patch)#patch_mask与patch大小一样
        ...
        patch_tmp = torch.zeros_like(imgs).cuda()#patch_tmp与imgs具有一样的大小元素全为零
        patch_mask_tmp = torch.zeros_like(imgs).cuda()#patch_temp与imgs具有一样的大小元素全为零
        ...
        for i in range(targets.size(0)):
            ...
            bbox_tl_x = bbox_x - bbox_w / 2#中心x的坐标
            bbox_tl_y = bbox_y - bbox_h / 2#中心y的坐标
            ...
            if patch_size == 0:
                patch_size = 1
            patch_resize = KT.resize(patch, (patch_size, patch_size), align_corners=True)#重塑大小
            patch_mask_resize = KT.resize(patch_mask, (patch_size, patch_size), align_corners=True)
            
            # temperature
            random_seed = round(random.uniform(-1, 1), 2)#四舍五入 保留最后两位
            ...
            patch2_padding = padding(patch_with_temp.clone())#对抗扰动边界填充
            patch2_mask_padding = padding(patch_mask_resize.clone())#掩码边界填充
        ...
        return patch_tf, patch_mask_tf#输出每张图片的对应的对抗扰动和掩码